...
for($x = 0; $x < $NUMBER_OF_DAYS; $x++) {
    ...
    $newTasks = mysql_query("Select Tasks.name as tName, Courses.name as cName, Tasks.creator, Tasks.due_date From Tasks, Courses, Users_Courses Where Tasks.course_id = index and student_id=$user_id and Tasks.course_id = Courses.index and DATEDIFF(CURDATE(), DATE(Tasks.created)) = $x");
    $editedTasks = mysql_query("Select Tasks.name as tName, Courses.name as cName, Tasks.creator, Tasks.due_date From Tasks, Courses, Users_Courses Where Tasks.course_id = index and student_id=$user_id and Tasks.course_id = Courses.index and DATEDIFF(CURDATE(), DATE(Tasks.modified)) = $x and Tasks.created != Tasks.modified");
